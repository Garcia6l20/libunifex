...
namespace unifex::net
{
  ...
  public:
    ...
    ipv4_endpoint() noexcept : m_address(), m_port(0) {}

    explicit ipv4_endpoint(
        ipv4_address address, std::uint16_t port = 0) noexcept
      : m_address(address)
      , m_port(port) {}

    [[nodiscard]] const ipv4_address& address() const noexcept {
      return m_address;
    }
    ...
    static std::optional<ipv4_endpoint>
