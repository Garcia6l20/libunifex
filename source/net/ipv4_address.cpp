...
#include <unifex/net/ipv4_address.hpp>
...
std::optional<unifex::net::ipv4_address>
unifex::net::ipv4_address::from_string(std::string_view string) noexcept {
  if (string.empty())
    return std::nullopt;
  ...
  // Could be a single 32-bit integer or first integer in a dotted decimal
  // string.
...

  {
    ...
    while (pos < length && local::is_digit(string[pos])) {
      ...
      if (partValue > maxValue ||
          (partValue == maxValue && digitValue > maxDigit)) {
        return std::nullopt;
      }
  ...
  for (int part = 1; part < 4; ++part) {
    ...
    const bool isPartOctal = (pos + 1) < length && string[pos] == '0' &&
        local::is_digit(string[pos + 1]);
    if (isPartOctal) {
      return std::nullopt;
    }
...
std::string unifex::net::ipv4_address::to_string() const {
