...
#include <unifex/net/ipv6_address.hpp>
...
std::optional<unifex::net::ipv6_address>
unifex::net::ipv6_address::from_string(std::string_view string) noexcept {
  ...
  while (pos < length && partCount < 8) {
    ...
    do {
      ...
      ++digitCount;
      ++pos;
    } while (digitCount < 4 && pos < length &&
             (digit = local::try_parse_hex_digit(string[pos])));

    // If we're not at the end of the string then there must either be a ':' or
    // a '.' next followed by the next part.
    if (pos < length) {
      ...
      if (string[pos] == ':') {
        ...
        if (digits[0] > 9 || (digitCount > 1 && digits[1] > 9) ||
        ...
        for (int decimalPart = 1; decimalPart < 4; ++decimalPart) {
          ...
          ++pos;
          digitCount = 1;
          while (digitCount < 3 && pos < length &&
                 local::is_digit(string[pos])) {
  ...
  // Finished parsing the IPv6 address, we should have consumed all of the
  // string.
  if (pos < length) {
    return std::nullopt;
  }
...
std::string unifex::net::ipv6_address::to_string() const {
