...
#include <unifex/net/ipv6_endpoint.hpp>
...
std::string unifex::net::ipv6_endpoint::to_string() const {
  std::string result;
  result.push_back('[');
...
std::optional<unifex::net::ipv6_endpoint>
unifex::net::ipv6_endpoint::from_string(std::string_view string) noexcept {
  ...
  auto address =
      net::ipv6_address::from_string(string.substr(1, closeBracketPos - 1));
  if (!address) {
    return std::nullopt;
  }
